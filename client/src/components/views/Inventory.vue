<template>
    <div class="inventory">
      <div class="grid">
        <div v-for="ingredient in ingredients" :key="ingredient.ingredientid" class="ingredient-card">
          ...
          <p><strong>Stock:</strong> {{ ingredient.stock }} {{ ingredient.units }}</p>
          <p><strong>Max Stock:</strong> {{ ingredient.maxstock }} {{ ingredient.units }}</p>
  ...
  export default {
    ...
    methods: {
      ...
      updateStock(item) {
        const newStock = prompt(`Enter new stock level for ${item.name}:`, item.stock)
        if (newStock !== null) {
          axios
      ...
      deleteItem(ingredientid) {
        if (confirm('Are you sure you want to delete this item?')) {
          axios
            .delete('http://localhost:3000/inventory/delete', { data: { ingredientid } })
            .then(() => this.fetchInventory())
      ...
      addNewItem() {
        ...
        if (newItem.ingredientid && newItem.name) {
          axios
            .post('http://localhost:3000/inventory/add', newItem)
            .then(() => this.fetchInventory())
